...
test("PF-007a: availability policy fails closed until operator confirmation is explicit", () => {
  const result = loadClinicCardAvailabilityPolicy({
    ...POLICY_ENV,
    CLINICCARD_AVAILABILITY_POLICY_CONFIRMED: "false",
  });
...
test("PF-007a: invalid calendar dates are rejected rather than rolled into another day", () => {
  ...
  const result = loadClinicCardAvailabilityPolicy({
    ...POLICY_ENV,
    CLINICCARD_CLOSED_DATES: "2026-02-31",
  });
  assert.equal(result.ok, false);
...
test("PF-007a: configured weekday and closed-date policy is deterministic", () => {
  const result = loadClinicCardAvailabilityPolicy({
    ...POLICY_ENV,
    CLINICCARD_CLOSED_DATES: "2026-08-24",
  });
...
test("PF-007a: explicitly closed date is authoritative empty availability with zero ClinicCard reads", async () => {
  let adapterCreated = false;
  const executor = createClinicCardAvailabilityExecutor({
    env: { ...POLICY_ENV, CLINICCARD_CLOSED_DATES: "2026-08-24" },
    adapterFactory: () => {
      adapterCreated = true;
      return emptyAdapter();
    },
...
test("PF-007a: generated slots come only from explicit hours and duration", async () => {
  ...
  const executor = createClinicCardAvailabilityExecutor({
    env: POLICY_ENV,
    adapterFactory: () => emptyAdapter(() => { listVisitsCalls += 1; }),
  });
  ...
  assert.deepEqual(result.data.slots, [
    {
      slot_id: "2026-08-24T10:00",
      starts_at: "2026-08-24T10:00:00",
      ends_at: "2026-08-24T11:00:00",
    },
    {
      slot_id: "2026-08-24T11:00",
      starts_at: "2026-08-24T11:00:00",
