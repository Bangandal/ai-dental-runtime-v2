...
export function buildRuntimeAgentSystemInstruction(opts?: RuntimeAgentSystemInstructionOptions): string {
  ...
  return [
    ...
    "   - Fill first_name and last_name by scanning EVERY prior turn of this conversation (including the very first message). If the patient gave their name at any point (e.g. 'меня зовут X Y', 'я X Y', or their name in passing), use it. Never ask the patient to repeat their name. If genuinely not found after looking through all prior turns, call booking.apply without first_name/last_name — the system will handle it.",
    ...
    "- booking.apply: create a visit. Call when patient has confirmed a slot and service. Fill first_name and last_name from ANYWHERE in this conversation (even from the earliest turns). If the patient said their name at any point in this conversation, use it. If not found after scanning all turns, call booking.apply without them — never ask for name when the patient has already said 'да' to a confirmation. Do not gate the call on having name in hand.",
    ...
    "- When calling booking.apply, the argument names for the patient's name are exactly first_name and last_name. Fill them using the patient's name from ANYWHERE in this conversation — INCLUDING the very first turns. If the patient stated their name at any turn (e.g. 'Николай Арманов', 'меня зовут Иван Петров', name given early in conversation), use it. Never ask the patient to repeat their name. If you cannot find it in ANY prior turn after thorough search, call booking.apply without first_name/last_name — the system handles it.",
    "- When the patient confirms a slot with 'да оформляйте', 'да', 'подходит', 'давайте', 'ок' — MANDATORY: call booking.apply immediately. Do NOT output a question. Do NOT ask for name or any other field. Scan the ENTIRE conversation history (all turns, including the earliest) for the patient's name and include it. If name is not found anywhere, call booking.apply without it.",
