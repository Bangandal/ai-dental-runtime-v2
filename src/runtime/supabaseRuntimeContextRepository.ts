...
import { deserializeBookingSubjects, type BookingSubjectsState } from "./bookingSubjectsState.ts";
    ...
    async loadRuntimeContext(input) {
      ...
      if (response.error) {
        return {
          ok: false,
          error: {
            code: "runtime_context_load_failed",
            message: String((response.error as { message?: unknown })?.message ?? response.error),
            retryable: true,
          },
        };
      }
      ...
      const providedPhone: ProvidedPhone | null =
        providedPhoneNumber && providedPhoneRaw?.phone_source === "typed"
          ? {
              ...
              phone_consent: false,
              phone_collected_at: asStr(providedPhoneRaw?.phone_collected_at) ?? loadedAt,
            }
          : null;
      ...
      return {
        ...
        data: {
          ...
          booking_subjects: deserializeBookingSubjects(stateJson?.booking_subjects),
          runtime_flags: {
