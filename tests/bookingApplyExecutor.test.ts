...
test("booking.apply: returns booking_write_disabled when CLINICCARD_LIVE_CLINIC_ALLOWLIST is unset", async () => {
  ...
  const executor = createBookingApplyExecutor({ env, adapterFactory: () => makeAdapter() });
  const result = await executor(makeContext());
  assert.equal(result.data.booking_status, "booking_write_disabled");
  assert.equal(result.data.may_claim_booked, false);
});
...
test("booking.apply: returns slot_conflict when doctor slot is taken", async () => {
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      listVisits: async () => ({ ok: true, data: [{
        id: 7, patient_id: 5, doctor_id: 1, cabinet_id: 9, date: "2026-07-15",
        time_start: "09:45", time_end: "10:15", status: "PLANNED",
      }] }),
    }),
  });
  const result = await executor(makeContext());
...
test("booking.apply: returns slot_conflict when cabinet is taken by a different doctor", async () => {
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      listVisits: async () => ({ ok: true, data: [{
        id: 8, patient_id: 5, doctor_id: 99, cabinet_id: 2, date: "2026-07-15",
        time_start: "10:00", time_end: "10:30", status: "PLANNED",
      }] }),
    }),
  });
  const result = await executor(makeContext());
...
test("booking.apply: no conflict when visit is for a different doctor and cabinet", async () => {
  ...
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      listVisits: async () => ({ ok: true, data: [{
        id: 8, patient_id: 5, doctor_id: 99, cabinet_id: 88, date: "2026-07-15",
        time_start: "10:00", time_end: "10:30", status: "PLANNED",
      }] }),
      createVisit: async (input) => {
        visitCreated = true;
        return { ok: true, data: {
          id: 100, patient_id: 42, doctor_id: input.doctor_id, cabinet_id: input.cabinet_id,
          date: input.date, time_start: input.time_start, time_end: input.time_end, status: input.status,
        } };
      },
    }),
  });
  ...
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      ...
      createVisit: async (input) => {
        visitInput = input;
        return { ok: true, data: {
          id: 99, patient_id: 42, doctor_id: input.doctor_id, cabinet_id: input.cabinet_id,
          date: input.date, time_start: input.time_start, time_end: input.time_end, status: input.status,
        } };
      },
    }),
...
test("booking.apply: may_claim_booked is false for all non-success paths", async () => {
  ...
  for (const c of cases) {
    const executor = createBookingApplyExecutor({ env: c.env ?? LIVE_ENV, adapterFactory: () => makeAdapter() });
...
test("booking.apply: returns cliniccard_write_failed when createPatient fails", async () => {
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      createPatient: async () => ({ ok: false, error: { code: "cliniccard_api_error", message: "Duplicate phone" } }),
    }),
  });
  const result = await executor(makeContext());
...
test("booking.apply: returns cliniccard_write_failed when createVisit fails", async () => {
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      createVisit: async () => ({ ok: false, error: { code: "cliniccard_api_error", message: "Doctor not available" } }),
    }),
  });
  const result = await executor(makeContext());
  assert.equal(result.data.booking_status, "cliniccard_write_failed");
...
test("booking.apply: returns cliniccard_write_failed when availability re-read fails", async () => {
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      listVisits: async () => ({ ok: false, error: { code: "cliniccard_http_error", message: "HTTP 503" } }),
    }),
  });
  const result = await executor(makeContext());
  assert.equal(result.data.booking_status, "cliniccard_write_failed");
...
test("booking.apply: computes time_end as time_start + configured 30 minutes", async () => {
  const executor = createBookingApplyExecutor({ env: LIVE_ENV, adapterFactory: () => makeAdapter() });
...
test("booking.apply: calls listVisits to re-read availability before every write", async () => {
  ...
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      listVisits: async () => { listVisitsCalled = true; return { ok: true, data: [] }; },
    }),
  });
  await executor(makeContext());
  assert.equal(listVisitsCalled, true);
});
...
test("booking.apply: reuses existing patient when findPatientByPhone returns a match", async () => {
  ...
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: true, data: [{ id: 55, name: "Ivan Petrov", phone: "+420777123456" }] }),
      createPatient: async () => { createPatientCalled = true; return { ok: true, data: { id: 99, name: "Should not reach here" } }; },
    }),
  });
  const result = await executor(makeContext());
...
test("booking.apply: calls createPatient when findPatientByPhone returns empty list", async () => {
  ...
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: true, data: [] }),
      createPatient: async (input) => { createPatientCalled = true; return { ok: true, data: { id: 42, name: input.name } }; },
    }),
  });
  const result = await executor(makeContext());
...
test("booking.apply: returns cliniccard_write_failed when findPatientByPhone fails", async () => {
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: false, error: { code: "cliniccard_http_error", message: "HTTP 502" } }),
    }),
  });
  const result = await executor(makeContext());
  assert.equal(result.data.booking_status, "cliniccard_write_failed");
...
test("INV-ID-01-5: own phone — no patients found → creates new patient", async () => {
  ...
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: true, data: [] }),
      createPatient: async (input) => { createPatientCalled = true; return { ok: true, data: { id: 88, name: input.name, phone: input.phone ?? null } }; },
    }),
  });
  const result = await executor(makeContext({ contact_phone_owner_subject_id: null }));
  assert.equal(result.data.booking_status, "visit_created");
  assert.ok(createPatientCalled);
...
test("INV-ID-01-7: borrowed phone — no candidates → creates new patient for target", async () => {
  ...
  const executor = createBookingApplyExecutor({
    ...
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: true, data: [] }),
      createPatient: async (input) => { createPatientCalled = true; return { ok: true, data: { id: 55, name: input.name, phone: input.phone ?? null } }; },
    }),
  });
...
test("INV-ID-01-8: borrowed phone — 1 name match → reuses matched patient", async () => {
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: true, data: [{ id: 33, name: "Ivan Petrov" }] }),
    }),
  });
  const result = await executor(makeContext({ contact_phone_owner_subject_id: "subject_1" }));
  assert.equal(result.data.booking_status, "visit_created");
...
test("INV-ID-01-9: borrowed phone — candidate name mismatch → creates new patient", async () => {
  ...
  const executor = createBookingApplyExecutor({
    env: LIVE_ENV,
    adapterFactory: () => makeAdapter({
      findPatientByPhone: async () => ({ ok: true, data: [{ id: 10, name: "Olena Koval" }] }),
      createPatient: async (input) => { createPatientCalled = true; return { ok: true, data: { id: 77, name: input.name, phone: input.phone ?? null } }; },
    }),
  });
  const result = await executor(makeContext({ contact_phone_owner_subject_id: "subject_1" }));
  assert.equal(result.data.booking_status, "visit_created");
...
test("AV-TRUTH-BOOK-1: live booking fails closed when explicit schedule config is missing", async () => {
  ...
  const executor = createBookingApplyExecutor({
    env,
    adapterFactory: () => makeAdapter({
      listVisits: async () => { clinicCardCalled = true; return { ok: true, data: [] }; },
    }),
  });
  const result = await executor(makeContext());
  assert.equal(result.data.booking_status, "config_missing");
...
test("AV-TRUTH-BOOK-2: booking uses configured duration instead of a hardcoded 30 minutes", async () => {
  const executor = createBookingApplyExecutor({
    env: { ...LIVE_ENV, CLINICCARD_SLOT_DURATION_MINUTES: "60" },
    adapterFactory: () => makeAdapter(),
  });
  const result = await executor(makeContext({ requested_time: "14:00" }));
  assert.equal(result.data.booking_status, "visit_created");
  assert.equal(result.data.time_start, "14:00");
  assert.equal(result.data.time_end, "15:00");
});

test("AV-TRUTH-BOOK-3: live booking rejects a configured holiday before any ClinicCard call", async () => {
  let clinicCardCalled = false;
  const executor = createBookingApplyExecutor({
    env: { ...LIVE_ENV, CLINICCARD_HOLIDAYS: "2026-07-15" },
    adapterFactory: () => makeAdapter({
      listVisits: async () => { clinicCardCalled = true; return { ok: true, data: [] }; },
    }),
  });
...
test("AV-TRUTH-BOOK-4: live booking rejects a slot whose duration crosses closing time", async () => {
  let clinicCardCalled = false;
  const executor = createBookingApplyExecutor({
    env: { ...LIVE_ENV, CLINICCARD_SLOT_DURATION_MINUTES: "60" },
    adapterFactory: () => makeAdapter({
  ...
  const result = await executor(makeContext({ requested_time: "17:30" }));
  assert.equal(result.data.booking_status, "invalid_slot");
  assert.equal(result.data.created_visit, false);
  assert.equal(clinicCardCalled, false);
});
